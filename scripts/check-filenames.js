...
function check() {
  ...
  for (const file of all) {
    ...
    if (rel.startsWith('components/') && /^use[A-Za-z0-9-]+\.(ts|tsx)$/.test(filename)) {
      violations.push({ rel, reason: 'Hook files are not allowed under components/*' })
      continue
    }

    // Enforce PascalCase only in components/heroui and components/providers
    if ((rel.startsWith('components/heroui/') || rel.startsWith('components/providers/'))) {
      if (legacyAllowlist.has(rel)) {
        // Skip legacy base modules
      } else if (!['index.ts', 'index.tsx'].includes(filename) && !pascalCaseRe.test(filename)) {
        violations.push({ rel, reason: 'Components must use PascalCase file names in heroui/providers' })
      }
    }
    ...
    if (rel.startsWith('lib/hooks/')) {
      if (legacyAllowlist.has(rel)) {
        ...
        violations.push({ rel, reason: "Hooks in lib/hooks must be camelCase and start with 'use' (useX.ts)" })
      }
    }
  }
