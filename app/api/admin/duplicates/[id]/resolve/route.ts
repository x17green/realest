...
export async function PUT(
  ...
  try {
    ...
    if (validatedData.action === 'keep_master' && !validatedData.master_property_id) {
      return NextResponse.json(
    ...
    if (validatedData.action === 'reject_duplicate' && !validatedData.rejection_reason) {
      return NextResponse.json(
    ...
    switch (validatedData.action) {
      ...
      case 'keep_master':
        ...
        const { data: keepMasterResult, error: keepMasterError } = await supabase
          .from('properties')
          .update({
            status: 'rejected',
      ...
      case 'reject_duplicate':
        ...
        const { data: rejectBothResult, error: rejectBothError } = await supabase
          .from('properties')
          .update({
            status: 'rejected',
            rejection_reason: validatedData.rejection_reason,
